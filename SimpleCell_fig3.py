'''To convert from fig 2 to fig 3:
- 2500ms run time
- during 1000-2000ms, sigma_IV was reduced from 4ms to 2ms, to indicate inh synchrony due to a stimulus

Copied from appendix on fig 3: 
The neuron was driven by a synchronous inhibitory input with aIV = 10, Δginh = 0.11 mS/ cm2, τinh = 10 ms, P = 26.10 ms, fosc = 38.3 Hz, CVT = 0.095, I = 2.4 μA/cm2, D = 0.04 mV2/ms. 
For t < 1000 and t > 2000, the spike time dispersion σIV was 4 ms, and, for 1000 ≤ t ≤ 2000, it was σIV = 2 ms. 
There was also a temporally homogeneous excitatory input with rate λexc = 1000 Hz, Δgexc = 0.02 mS/cm2 and τexc = 2 ms. 
The subthreshold membrane potential in (b) was obtained by reducing current to I = −2.0 μA/cm2 to prevent action potentials.
The firing rate histogram was calculated based on 500 trials.


- graph A: membrane ptl
- graph B: local field ptl
    - in the fig 2 recreation, we replaced this with a membrane ptl graph, but the excitatory current injection was decreased to prevent action ptls
- graph C: sp/s bin plot
- graph D: rastergram of 10 trials


For now:
- Make the simulation run for 2500ms. I increased num_spike_volleys from 80 to 100. I increased runtime to 2500
- Make the sigma change to 4 ms outside and 2 ms inside the stimulus
- Make a new synapse w/ excitatory input
- increase delta_g_inh from 0.044 to 0.11 mS/cm^2
- decrease I from 4.0 to 2.4 µA/cm^2
- for graph B, reduce I from 2.4 to -2.0 µA/cm^2

Next:
- Make it run for 500 trials, calculate the sp/s histogram on this, but still only plot the first 10 on the raster plot


Questions:
- Did I do the temporally homogenous excitatory input correctly?
- decrease a_IV from 25 to 10? Where do we mention a_IV?
- decrease D from 0.08 to 0.04 mV^2/ms? Where do we use D?
...
def generate_spike_times():
    ...
    for v_time in range(1, len(spike_volleys) - 1):
        ...
        if (spike_volleys[v_time] > 1000) and (spike_volleys[v_time] < 2000): #sigma_iv is 8ms before 300 and after 700. It is 2ms within this interval
            # sigma_IV = 2
            ...
            # sigma_IV = 8
            gaus = gaussian_density(x, spike_volleys[v_time], 4)
...
# stdev = CV_t * mean
num_spike_volleys = 100 #the number of spike volleys generated
...
current = 2.4 # (uA/cm2) from fig 2 in appendix A.4. For 2a it's 4, for 2b it's 1
...
# print()
# print(bin_counter)
axs[2].hist(bin_x_values, weights=bin_counter, bins=len(bin_x_values)//10)
